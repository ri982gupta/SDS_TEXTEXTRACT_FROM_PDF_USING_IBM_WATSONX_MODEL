#------------------------------------------ FIND CHEMICAL WEIGHT OF CHEMICAL NAME-----------------------------------------------
...
def extract_details_with_llm(text_chunks, chemical_name, llama_model):
    for chunk in text_chunks:
        prompt = f"Extract the chemical weight for the chemical name '{chemical_name}' from the following text:\n{chunk}"
...
def extract_ghs_statement(chemical_name, pdf_path, llama_model):
    ...
    if pdf_text:
        ...
        response_json = {
            "ChemicalDetails": {
                "Name": chemical_name,
                "Chemical weight": details if details else "not found"
            }
        }
